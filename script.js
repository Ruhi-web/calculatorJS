...
printOutput("9373874");
function reverseSetNumberFormat(num){
...
for (var i = 0; i < operator.length; i++) {
    operator[i].addEventListener('click', function(){
        ...
        var output=reverseSetNumberFormat(getOutput).toString();
...
for (var i = 0; i < number.length; i++) {
    number[i].addEventListener('click', function(){
        var output=reverseSetNumberFormat(getOutput());
        if (output!=NaN) {
